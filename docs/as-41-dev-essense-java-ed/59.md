...
在“名称”字段中输入 ServiceExample，并将 com . ebookwidge . service example 指定为包名称。在单击完成按钮之前，将最低API级别设置更改为API 26:安卓 8.0(奥利奥)，并将语言菜单更改为 Java。
...
如前一节所述，在主线程上执行耗时操作的任何组件都会导致应用在该任务完成之前变得无响应。因此，安卓提供了一个允许应用创建和使用额外线程的API并不奇怪。在独立于主线程的线程中执行的任何任务本质上都是在后台执行的。这种线程通常被称为后台线程或工作线程。
