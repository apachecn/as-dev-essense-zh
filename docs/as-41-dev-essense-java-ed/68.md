...
首先启动AndroidStudio，并打开上一章开始的 RoomDemo 项目。在向项目中添加任何新类之前，第一步是向构建配置中添加一些额外的库，包括房间持久性库。找到并编辑模块级 build.gradle 文件(app-> Gradle Scripts-> build . Gradle(Module:roodemo . app))并按如下方式进行修改:
...
如先前在[“安卓房间持久性库”](66.html#_idTextAnchor1241)中所述，数据库检查器工具窗口可用于检查与正在运行的应用相关联的房间数据库的内容，并执行微小的数据更改。使用 RoomDemo 应用添加一些数据库记录后，使用[图 68-3](#_idTextAnchor1303) 中指示的选项卡按钮显示数据库检查器工具窗口:
