# 四、在AndroidStudio创建安卓虚拟设备(AVD)

在AndroidStudio开发安卓应用的过程中，需要多次编译和运行一个应用。安卓应用可以通过在物理设备或安卓虚拟设备模拟器环境中安装和运行来测试。在使用 AVD 之前，必须首先创建和配置它，以匹配特定设备型号的规格。因此，本章的目标是使用 Pixel 3 手机作为参考示例，完成创建此类虚拟设备的步骤。

4.1 关于安卓虚拟设备

avd 本质上是模拟器，允许测试 Android 应用，而无需将应用安装在基于 Android 的物理设备上。AVD 可以被配置成模拟各种硬件特征，包括诸如屏幕尺寸、存储器容量以及诸如照相机、全球定位系统导航支持或加速度计等特征的存在或不存在的选项。作为标准AndroidStudio安装的一部分，安装了许多仿真器模板，允许为一系列不同的设备配置自动增值设备。通过指定处理器类型、内存容量以及屏幕大小和像素密度等属性，可以创建自定义配置来匹配任何物理安卓设备。

AVD 会话可以作为单独的窗口出现，也可以嵌入AndroidStudio窗口。[例如，图 4-1](#_idTextAnchor073) 显示了一个配置为模拟 Google Pixel 3 模型的 AVD 会话。
...
要创建一个新的 AVD ，第一步是启动 AVD 管理器。这可以在AndroidStudio环境中通过从主窗口中选择工具- > AVD 管理器菜单选项来实现。
...
虽然模拟器可能默认以纵向显示，但这个和其他默认选项可以更改。在 AVD 管理器中，选择新的 Pixel 4 条目，然后单击设备行的“操作”列中的铅笔图标。在配置屏幕中，找到启动方向部分并更改方向设置。退出并重新启动模拟器会话，以查看此更改是否生效。模拟器的更多细节将在下一章中介绍([“使用和配置AndroidStudio AVD 模拟器”](05.html#_idTextAnchor102))。
...
点击运行按钮后，AndroidStudio将在选定的模拟器和设备上启动该应用。
...
安卓 10 引入了人们期待已久的黑暗主题，默认情况下，AndroidStudio应用项目不支持该主题。要在 AVD 模拟器中测试黑暗主题，请在模拟器中运行的安卓实例中打开设置应用。有许多不同的方法可以访问设置应用。最快的方法是显示主屏幕，然后单击并从屏幕底部向上拖动(就在搜索栏下方)。这将显示设备上安装的所有应用，其中一个是设置应用。
...
到目前为止，在本章中，我们只使用模拟器作为独立的窗口。模拟器也可以作为嵌入AndroidStudio主窗口的工具窗口运行。要嵌入模拟器，请选择文件->设置...菜单选项(AndroidStudio->首选项...在 macOS 上)，导航到首选项对话框左侧导航面板中的工具->仿真器，并启用在工具窗口中启动选项:
...
再次运行示例应用，此时模拟器将出现在AndroidStudio窗口内，如图[图 4-15](#_idTextAnchor096) :
...
1.启动AndroidStudio，打开本书前面创建的安卓样本项目。

2.选择文件->项目结构...菜单选项。

3.复制“项目结构”对话框的“JDK 位置”字段中包含的路径。这代表了与AndroidStudio捆绑的 JRE 的位置。
...
AVD 模拟器会话可以显示为独立窗口，也可以嵌入主AndroidStudio用户界面。