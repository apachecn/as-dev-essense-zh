...
本章的其余部分将用于演示设备上安装的多个活动与隐含意图的要求相匹配的效果。为此，将创建第二个应用并将其安装在设备或仿真器上。因此，首先在AndroidStudio中创建一个新项目，将应用名称设置为我的网络视图，使用本章前面创建隐含内容项目时使用的相同的软件开发工具包配置选项，并再次选择一个空活动。
