...
在标题为[“带有 Jetpack 的现代安卓应用架构”](32.html#_idTextAnchor696)的章节中，我们介绍了安卓数据绑定的概念，并简要解释了如何使用它将用户界面布局中的视图直接连接到位于应用内其他对象中的方法和数据，而无需编写代码。本章将提供更多关于数据绑定的细节，重点解释数据绑定是如何在 AndroidStudio 项目中实现的。下一章的教程([“一个安卓 Jetpack 数据绑定教程”](36.html#_idTextAnchor751))将提供一个数据绑定的实际例子。
