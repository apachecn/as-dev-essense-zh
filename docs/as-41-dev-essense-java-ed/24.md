...
前一章介绍了使用 ConstraintLayout 和 ConstraintSet 类在 Java 代码中创建和修改用户界面布局的基本概念。本章将采用这些概念，并通过创建一个完全用 Java 代码创建的示例布局，而不使用AndroidStudio布局编辑器工具，将它们付诸实践。

24.1 在AndroidStudio创建示例项目

启动AndroidStudio，从欢迎屏幕的快速开始菜单中选择创建新项目选项，并在生成的新项目对话框中选择空活动模板，然后单击下一步按钮。

在“名称”字段中输入 JavaLayout，并将 com . ebookwidue . JavaLayout 指定为包名。在单击完成按钮之前，将最低API级别设置更改为API 26:安卓 8.0(奥利奥)，并将语言菜单更改为 Java。

创建项目后，MainActivity.java 文件将自动加载到编辑面板中。正如我们所料，AndroidStudio创建了一个模板活动，并覆盖了 onCreate()方法，为添加 Java 代码来创建用户界面提供了一个理想的位置。
...
本练习的下一个任务是将 EditText 视图的宽度设置为 200dp。如标题为[“AndroidStudio布局编辑器约束布局教程”](21.html#_idTextAnchor513)的章节所述，在用户界面布局中设置大小和位置时，最好使用密度无关像素(dp)而不是像素(px)。为了使用 dp 设置位置，有必要在运行时将 dp 值转换为 px 值，同时考虑设备显示的密度。因此，为了将 EditText 视图的宽度设置为 200dp，需要向类中添加以下代码:
...
当然，本章中创建的示例活动创建了一个类似于早期[“AndroidStudio中的手动 XML 布局设计”](22.html#_idTextAnchor539)一章中创建的用户界面(尽管背景颜色和视图类型发生了变化)。如果没有别的，这一章应该对AndroidStudio布局编辑器工具和 XML 资源在多大程度上保护开发人员免受创建安卓用户界面布局的许多复杂性的影响有所了解。
