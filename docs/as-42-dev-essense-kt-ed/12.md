# 十二、Kotlin数据类型、变量和可空性

这一章和接下来的几章都旨在介绍 Kotlin 编程语言的基础知识。本章将重点介绍 Kotlin 代码中可用的各种数据类型。这也将包括对常量、变量、类型转换和Kotlin空值处理的解释。

正如前一章[“Kotlin简介”](11.html#_idTextAnchor250)中所概述的，一个有用的语言实验方法是使用Kotlin在线游戏环境。因此，在开始这一章之前，打开一个浏览器窗口，导航到[https://play.kotlinlang.org](https://play.kotlinlang.org/)，并使用操场在这一章和后面的其他Kotlin介绍性章节中尝试代码。
...
Kotlin字符数据类型用于存储渲染文本的单个字符，如字母、数字、标点符号或符号。Kotlin 中的内部字符以 16 位 Unicode 字形簇的形式存储。字形簇由两个或多个 Unicode 代码点组成，它们组合在一起表示一个可见字符。
...
注意在给变量赋值时使用单引号。这向Kotlin表明，这是一个字符数据类型，而不是双引号表示字符串数据类型。
...
变量本质上是计算机内存中为存储应用使用的数据而保留的位置。程序员给每个变量起一个名字，并赋予一个值。然后，分配给变量的名称可以在Kotlin代码中使用，以访问分配给该变量的值。这种访问可以包括读取变量的值，或者在可变变量的情况下，更改值。
