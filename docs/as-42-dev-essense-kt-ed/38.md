...
在“名称”字段中输入 FragmentExample，并将 com . ebookwidge . FragmentExample 指定为包名。在单击完成按钮之前，将最低 API 级别设置更改为 API 26:安卓 8.0(奥利奥)，并将语言菜单更改为 Kotlin。使用第 [18.8 节“将项目迁移到视图绑定”](18.html#_idTextAnchor393)中概述的步骤，修改项目以使用视图绑定。
...
这个用户界面将由一个 XML 布局文件和一个片段类组成。虽然这些可以手动添加，但让 AndroidStudio 为我们创建它们会更快。在项目工具窗口中，找到 app-> Java-> com . ebookwidue . fragmentexample 条目并右键单击它。从结果菜单中，选择新建->片段->图库...显示如下图 38-1 所示对话框的选项:
...
与空活动模板一样，当新片段添加到项目中时，AndroidStudio 4.2 不支持视图绑定。因此，在进入本教程的下一步之前，我们需要执行迁移。首先编辑 ToolbarFragment.kt 文件，然后导入片段的绑定，如下所示:
...
当用户触摸工具栏片段中的按钮时，片段类将需要从编辑文本视图中获取文本和 SeekBar 的当前值，并将它们发送到文本片段。正如[《安卓片段介绍》](37.html#_idTextAnchor800)中所概述的，片段之间不应该直接通信，而应该使用嵌入它们的活动作为中介。
