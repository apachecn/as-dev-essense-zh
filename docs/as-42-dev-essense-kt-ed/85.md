...
![](image/as_4.1_custom_print_ui.jpg)
...
从安卓应用程序中打印定制文档的大部分工作都涉及到定制打印适配器的实现。此示例将需要一个实现了 onLayout()和 onWrite()回调方法的打印适配器。在 MainActivity.kt 文件中，为这个新类添加模板，使其如下所示:
...
请注意，在本例中，将打印一份四页的文档。在更复杂的情况下，应用程序很可能需要根据与用户的纸张大小和页面方向选择相关的内容的数量和布局来动态计算要打印的页数。
...
![](image/as_4.1_custom_print_running.jpg)
