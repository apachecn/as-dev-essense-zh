...
在名称字段中输入发送广播，并将 com . ebookwidue . send broadcast 指定为包名称。在单击完成按钮之前，将最低API级别设置更改为API 26:安卓 8.0(奥利奥)，并将语言菜单更改为Kotlin。
...
从代码中可以看出，AndroidStudio已经为新类生成了一个模板，并为 onReceive()方法生成了一个存根。现在需要对类进行大量的更改来实现所需的行为。因此，保留在 MyReceiver.kt 文件中，修改代码，使其如下所示:
