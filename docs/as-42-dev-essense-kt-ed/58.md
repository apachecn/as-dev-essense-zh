# 五十八、AndroidStudio MasterDetailFlow教程

本章将解释主/细节用户界面设计的概念，然后详细探讨AndroidStudio中包含的MasterDetailFlow模板的组成元素。然后将创建一个示例应用，演示修改模板以满足应用开发人员的特定需求所涉及的步骤。
...
在“名称”字段中输入 PrimaryDetailFlow，并将 com . ebookwidge . PrimaryDailFlow 指定为包名。在单击完成按钮之前，将最低API级别设置更改为API 26:安卓 8.0(奥利奥)，并将语言菜单更改为Kotlin。
...
AndroidStudio为 fragment _ 网站 _detail.xml 提供的模板布局包括一个包含应用栏、工具栏、浮动操作按钮和文本视图的协调器布局。对于这个例子，我们只需要显示一个网络视图，所以打开这个文件，从布局中删除除了网站 _ 细节 _ 容器视图之外的所有视图。最简单的方法是在点击键盘删除键之前，选择组件树中所有不需要的视图，如下所示:
...
主/详细用户界面由项目的主列表组成，当选择该列表时，将在详细信息窗格中显示关于该选择的附加信息。主/细节流是AndroidStudio提供的一个模板，允许快速且相对容易地创建主/细节安排。正如本章中所展示的，通过对默认模板文件的微小修改，可以用最少的编码和设计工作实现广泛的基于主要/细节的功能。