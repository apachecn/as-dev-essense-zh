...
从用户的角度来看，存储访问框架提供了一个直观的用户界面，允许用户从安卓应用中浏览、选择、删除和创建由存储服务(也称为文档提供者)托管的文件。使用该浏览界面(也称为选择器，用户可以浏览由他们选择的文档提供商托管的文件(如文档、音频、图像和视频)。[例如，图 76-1](#_idTextAnchor1442) 显示了显示由文档提供商服务托管的文件集合的选择器用户界面:
...
执行上述代码时，将显示选择器用户界面，允许用户浏览和选择由可用文档提供者托管的任何文件。一旦用户选择了一个文件，对该文件的引用将以 Uri 对象的形式提供给应用。然后，应用可以使用 openFileDescriptor(Uri，String)方法打开文件。但是，也有一些风险，不是所有由文档提供者列出的文件都可以用这种方式打开。通过使用 CATEGORY _ OPERATION选项修改意图，可以在选择器中排除此类文件。例如:
