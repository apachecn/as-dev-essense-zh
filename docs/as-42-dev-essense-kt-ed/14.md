...
不管使用哪种编程语言，应用开发在很大程度上是一种应用逻辑的练习，编程艺术的很大一部分涉及编写基于一个或多个标准做出决策的代码。这样的决定定义了哪些代码被执行，执行了多少次，反过来，哪些代码在程序执行时被忽略。这通常被称为流程控制，因为它控制程序执行的流程。流控制通常分为循环控制(代码执行频率)和条件流控制(代码是否执行)。本章旨在介绍科特林的两种流量控制。
