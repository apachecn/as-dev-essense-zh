...
在名称字段中输入视图模型演示，并将 com . ebookwidge . ViewModelDemo 指定为包名。在单击完成按钮之前，将最低API级别设置更改为API 26:安卓 8.0(奥利奥)，并将语言菜单更改为科特林。编辑 build.gradle(模块:视图模型演示)文件，启用视图绑定，然后单击编辑器面板顶部的立即同步链接:
