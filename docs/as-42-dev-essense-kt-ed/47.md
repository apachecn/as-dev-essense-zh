...
每个应用程序都有一个主屏幕，在应用程序启动后和任何闪屏出现后都会出现(闪屏是应用程序加载时临时出现的应用程序品牌屏幕)。在这个主屏幕上，用户通常会执行导致其他屏幕出现的任务。这些屏幕通常会采取应用程序中其他活动和片段的形式。例如，消息应用程序可能有一个主屏幕，列出当前消息，用户可以从中导航到另一个屏幕以访问联系人列表或设置屏幕。反过来，联系人列表屏幕可能允许用户导航到可以添加新用户或更新现有联系人的其他屏幕。图形上，应用程序的导航图可能如图 47-1 所示:

![](image/jetpack_navigation_graph_diagram.jpg)

图 47-1

组成应用程序的每个屏幕，包括主屏幕，都被称为目的地，通常是一个片段或活动。安卓导航架构使用导航栈来跟踪用户在应用程序中通过目的地的路径。当应用程序第一次启动时，主屏幕是放置在堆栈上的第一个目的地，并成为当前目的地。当用户导航到另一个目的地时，该屏幕将成为当前目的地，并被推送到主目的地上方的堆栈上。当用户导航到其他屏幕时，它们也被推送到堆栈上。[例如，图 47-2](#_idTextAnchor933) 显示了用户启动应用程序并导航到“添加联系人”屏幕后，假设的消息应用程序的导航堆栈的当前状态:
...
[图 47-5](#_idTextAnchor939) ，显示设计模式下的安卓工作室导航图编辑器:
